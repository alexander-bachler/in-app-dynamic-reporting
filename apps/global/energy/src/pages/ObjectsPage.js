...
const ObjectsPage = ({
    ...
    useEffect(() => {
        const fetchObjectTree = async () => {
            setLoading(true);
            try {
                const tree = await apiClient.getObjectTree();
                setObjectTree(tree);
            } catch (error) {
    ...
    const handleObjectChange = async (event) => {
        ...
        if (objectId) {
            setLoading(true);
            try {
                const response = await apiClient.getMeasuringPoint(objectId);
    ...
    const fetchDataForMeasuringPoint = async () => {
        if (selectedMeasuringPointId && dateFrom && dateTo) {
            ...
            const response = await apiClient.getDataForMeasuringPoint(selectedMeasuringPointId, timeFrom, timeTo, granularity);
